...
public class SearchRemittance extends ViewPayments {
	...
	@FindBy(xpath = "//td[@class='commenlink' and @id='homeId']/a[1]")
	WebElement HomeBtn;
	...
	WebDriverWait wait = new WebDriverWait(testConfig.driver, 20);
	...
	public SearchRemittance(TestBase testConfig) {
		...
		// Element.expectedWait(divSearchCriteria, testConfig, "Search Criteria
	...
	public void verifySearchResults(String requestType) throws IOException, InterruptedException, JAXBException,
		...
		if (testConfig.getRunTimeProperty("testSuite").equals("UPA")) {
			Helper.compareEquals(testConfig, "Record Count from FISL and UI :", totalRecordsFromFISL,
					getRecordCountFromUISR());
			if (!totalRecordsFromFISL.equalsIgnoreCase("0"))
				Helper.compareMaps(testConfig, "Payments Details Comparison ",
						getSRDetailsFromFISL(requestType, searchResponse), getSRDetailsFromUI(requestType));
			else
				Element.verifyTextPresent(errorMsg,
						"No records match the selected search criteria. Choose a different search option or try your search again later.");
			// Helper.compareEquals(testConfig, "Record Count from FISL and DB
			// :",totalRecordsFromFISL,getRecordCountFromDB(requestType));
		} else
			Helper.compareMaps(testConfig, "Payments Details Comparison ",
					getSRDetailsFromFISL(requestType, searchResponse), getSRDetailsFromUI(requestType));
	...
	public void verifyPaymentDetailsForCSR(String requestType, EpsPaymentsSummarySearchResponse searchResponse)
			throws JAXBException, IOException, SAXException, ParserConfigurationException, ParseException {
		String totalRecordsFromFISL = String.valueOf(searchResponse.getData().getTotalCount());
		if (!totalRecordsFromFISL.equalsIgnoreCase("0"))
			Helper.compareLinkedMaps(testConfig, "Payments Details Comparison ",
					getPaymentDetailsFromFISLForCSR(searchResponse), getPaymentDetailsFromCSRUI());
		else
			Element.verifyTextPresent(CSRErrorMsg,
					"No records match the selected search criteria. Choose a different search option or try your search again later.");
	...
	public Object createRequest(String requestType) throws JAXBException {
		EpsSearchRemittanceRequestHelper epsSearchRemittanceRequestHelper = new EpsSearchRemittanceRequestHelper(
				requestType);
		EpsPaymentsSearchRequest epsSearchRemittanceSearchRequest = epsSearchRemittanceRequestHelper
				.createRequestPojo();
		return epsSearchRemittanceSearchRequest;
	}

	public EpsPaymentsSearchRequest setServiceData(Object object)
	...
	public Object getFISLResponse1(String requestType)
		...
		if (requestType.equals("byDOPAndPatientNm") || requestType.equals("byElectronicPaymentNo")
				|| requestType.equals("byCheckNo"))
		...
		EpsPaymentsSummarySearchResponse searchResponse;
		searchResponse = (EpsPaymentsSummarySearchResponse) epsSearchRemittanceRequestHelper
				.postRequestGetResponse(epsSearchRemittanceSearchRequest);

		return searchResponse;
	}

	public EpsPaymentsSearchRequest setToAndFromDateDOS(Object object)
			throws JAXBException, IOException, SAXException, ParserConfigurationException {
		((EpsPaymentsSearchRequest) object).getClaimServiceDateRange()
	...
	public EpsPaymentsSearchRequest setMapEntryKey1(Object object)
			throws JAXBException, IOException, SAXException, ParserConfigurationException {
	...
	public EpsPaymentsSearchRequest setMapEntryKey2(Object object)
			throws JAXBException, IOException, SAXException, ParserConfigurationException {
		if (testConfig.getRunTimeProperty("key1") != null) {

			((SearchByCriteriaRequest) object).getSearchCriteria().getParameterMap().getEntries().get(1)
					.setKey(testConfig.getRunTimeProperty("key1"));
			((SearchByCriteriaRequest) object).getSearchCriteria().getParameterMap().getEntries().get(1)
					.setValue(testConfig.getRunTimeProperty("value1"));
		}
		return (EpsPaymentsSearchRequest) object;
	}
	...
	public List<String> getColumnValue(String colName) {
		...
		if (colName.equals("Archive"))
			for (int i = 2; i < noOfRows; i++) {
				p = true;
				divSearchResults = Element.findElements(testConfig, "xpath", "//div[@id='SearchHeader']//table//tr");
				Map attMap = Element.getAllAttributes(testConfig, divSearchResults.get(i).findElements(By.tagName("td"))
						.get(colNumber).findElement(By.tagName("input")), "Access Level Attributes");
				String result = (String) attMap.get("CHECKED");
				if (result == null)
					result = attMap.containsKey("checked") ? "true" : "false";
				if (result.equals("false") && flag == 0) {
					p = false;
					divSearchResults.get(i).findElements(By.tagName("td")).get(colNumber)
							.findElement(By.tagName("input")).click();
					Browser.wait(testConfig, 1);
					btnSaveArchive.click();
					Element.expectedWait(divSearchCriteria, testConfig, "Search Criteria section",
	...
	public void verifySortingOrder(WebElement lnkName, String colName, String criteriaType)
		...
		case "Patient Name":
			 ...
			 * 
			 * @Override public int compare(ArrayList<String> a1,ArrayList<String> a2) { int
			 * v1=a1.get(2).compareTo(a2.get(2)); int v2=a1.get(1).compareTo(a2.get(1)); int
			 * v3=a1.get(3).compareTo(a2.get(3)); if(v1==0) return (v2==0)?v3:v2; else
			 * return v1; }
			 * 
			 * }); if(listString.size()<30) for(int i=0;i<listString.size();i++)
			 * newList.add(listOfPatients.get(0).get(0)); else for(int i=0;i<30;i++)
	...
	public void verifyPaymentStatus(String type) {
		...
		if (divSearchResults.size() != 0) {
			for (rowNo = 2; rowNo < divSearchResults.size(); rowNo++) {
				if (divSearchResults.get(rowNo).findElements(By.tagName("td")).get(10).getText().toString()
						.equals("ACH") && ACHflag == 0) {
					Helper.compareContains(testConfig, "Verify Status Code", "Successful ACH",
							divSearchResults.get(rowNo).findElements(By.tagName("td")).get(11).getText());
					ACHflag = 1;
				} else if (divSearchResults.get(rowNo).findElements(By.tagName("td")).get(10).getText().toString()
						.equals("CHK") && CHKflag == 0) {
					Helper.compareContains(testConfig, "Verify Status Code", "Successful CHK",
							divSearchResults.get(rowNo).findElements(By.tagName("td")).get(11).getText());
					CHKflag = 1;
				} else if (divSearchResults.get(rowNo).findElements(By.tagName("td")).get(10).getText().toString()
						.equals("NON") && NONflag == 0) {
					Helper.compareContains(testConfig, "Verify Status Code", "N/A",
							divSearchResults.get(rowNo).findElements(By.tagName("td")).get(11).getText());
					NONflag = 1;
				} else if (divSearchResults.get(rowNo).findElements(By.tagName("td")).get(10).getText().toString()
						.equals("VCP") && NONflag == 0) {
					Helper.compareContains(testConfig, "Verify Status Code", expectedStatus,
							divSearchResults.get(rowNo).findElements(By.tagName("td")).get(11).getText());
					NONflag = 1;
				}
			}
			...
			if (divSearchResults.get(rowNo).findElements(By.tagName("td")).get(6).getText().equals(type))
				Helper.compareContains(testConfig, "Verify Status Code", expectedStatus,
						divSearchResults.get(rowNo).findElements(By.tagName("td")).get(13).getText());
		}
	...
	public String getStatus(String typeCode) {
		...
		case "VCP":
			if (testConfig.getRunTimeProperty("paymentMethCode").equals("NON")
					&& testConfig.getRunTimeProperty("paymentStatusTypeID") != null)
				return "N/A";
			else
	...
	public void verifyReturnedReasonDisplayed(String criteriaType) {
		...
		if (testConfig.getRunTimeProperty("testSuite").equalsIgnoreCase("CSR")) {
			divSearchResults = Element.findElements(testConfig, "xpath",
					".//*[@id='searchRemittanceResultsForm']/table/tbody/tr[7]/td/table/tbody/tr/td/table/tbody/tr"); // .//*[@id='searchRemittanceResultsForm']//tr[7]//tr[2]/td[13]/span
			...
			case "byElectronicPaymentNo":
				...
				Helper.compareContains(testConfig, "verify Column Data", colData,
						testConfig.getRunTimeProperty("returnedReason"));
				break;
			case "byCheckNo":
	...
	public void verifyPrintFirstPage() throws IOException, AWTException, InterruptedException {
		...
		// FileInputStream("C:\\Users\\akushw10\\Downloads\\Search_Remit_Report.xls");
	...
	public Map<String, LinkedHashMap<String, String>> getSRDetailsFromUI(String requestType) {
		...
		if (requestType.equalsIgnoreCase("byElectronicPaymentNo") || requestType.equalsIgnoreCase("byCheckNo")
				|| requestType.equalsIgnoreCase("byDOPAndNpi"))
			startingLoop = 1;
		...
		if ("printPaymentSummary".equals(testConfig.getRunTimeProperty("page"))) {
			divSearchResultsUPA = Element.findElements(testConfig, "xpath",
					"//*[@id='SearchHeader']/table/tbody/tr/td/table/tbody/tr");
		} else {
		...
		if (requestType.equalsIgnoreCase("byElectronicPaymentNo") || requestType.equalsIgnoreCase("byCheckNo")) {
			if ("printPaymentSummary".equals(testConfig.getRunTimeProperty("page"))) {
				...
				divSearchResultsUPA = Element.findElements(testConfig, "xpath",
						"//*[@id='search-remmitance']/table/tbody/tr[7]/td/table/tbody/tr/td/table/tbody/tr");
			}
		}
		if (requestType.equalsIgnoreCase("byDOPAndNpi") || requestType.equalsIgnoreCase("byDOSAndNpi")) {
			if ("printPaymentSummary".equals(testConfig.getRunTimeProperty("page"))) {
				...
				divSearchResultsUPA = Element.findElements(testConfig, "xpath",
						"//*[@id='search-remmitance']/table/tbody/tr[8]/td/table/tbody/tr/td/table/tbody/tr");
			}
		}
		...
		for (int pageNo = 1; pageNo <= totalNoOfPages; pageNo++) {
			for (int i = startingLoop; i < divSearchResultsUPA.size(); i++) {
				...
				if (requestType.equalsIgnoreCase("byElectronicPaymentNo") || requestType.equalsIgnoreCase("byCheckNo")
						|| requestType.equalsIgnoreCase("byDOPAndNpi") || requestType.equalsIgnoreCase("byDOSAndNpi")) {
					...
					// (innerMap.get("Type").equals("NON")||innerMap.get("Type").equals("DD")))
				...
				// innerMap.put("Payment Status",innerMap.get("Payment Status /
				// Trace Number"));
				...
				outerMap.put(innerMap.get("Payment Number") + "_" + innerMap.get("Patient Name") + "_"
						+ innerMap.get("Subscriber ID") + "_" + cl, innerMap);
	...
	public LinkedHashMap<String, String> getPaymentDetailsFromCSRUI() {
		...
		// if(divSearchResults.size()==0)
		// divSearchResults=Element.findElements(testConfig,
		// "xpath",".//*[@id='search-remmitance']/table/tbody/tr[8]/td/table/tbody/tr/td/table//tr");
			...
			for (int i = 1; i < divSearchResults.size(); i++) {
				...
				for (int j = 0; j < headers.size(); j++) {
					...
					if (headers.get(j).equals("Payment Status/Trace Number"))
						innerMap.put(headers.get(j),
								divSearchResults.get(i).findElements(By.tagName("td")).get(13).getText());
	...
	public Map<String, LinkedHashMap<String, String>> getPaymentDetailsFromCSRUI(String searchType) {
		...
		// if(divSearchResults.size()==0)
		divSearchResults = Element.findElements(testConfig, "xpath",
				".//*[@id='searchRemittanceResultsForm']/table/tbody/tr[8]/td/table/tbody/tr/td/table//tr");
		...
		for (int pageNo = 1; pageNo <= totalNoOfPages; pageNo++) {
			for (int i = 1; i < divSearchResults.size(); i++) {
				...
				for (int j = 0; j < headers.size(); j++) {
					...
					if (headers.get(j).equals("Market Type"))
						innerMap.put(headers.get(j),
								divSearchResults.get(i).findElements(By.tagName("td")).get(12).getText());
					else
						innerMap.put(headers.get(j), details);
				}
	...
	public Map<String, LinkedHashMap<String, String>> getSRDetailsFromFISL(String requestType, Object FISLResponse)
			throws JAXBException, IOException, SAXException, ParserConfigurationException, ParseException {
		int totalPayments;
		LinkedHashMap<String, String> innerMap = null;
		...
		for (int i = 0; i < totalPayments; i++) {
			...
			if (requestType.equals("byDOP") || requestType.equals("byElectronicPaymentNo")
					|| requestType.equalsIgnoreCase("byCheckNo") || requestType.equalsIgnoreCase("byDOPAndAccountNo"))
			...
			String patientName = payments[i].getPatientFirstName() + " " + payments[i].getPatientMiddleName() + " "
					+ payments[i].getPatientLastName();
			patientName = patientName.replace("null ", "").trim();

			if (requestType.equals("byDOPAndAccountNo") || requestType.equals("byDOP")
					|| requestType.equals("byDOPAndRenderingProvider") || requestType.equals("byDOPAndPatientNm")
					|| requestType.equals("byDOSAndAcntNo") || requestType.equals("byDOS")
					|| requestType.equals("byDOSAndRenderingProvider") || requestType.equals("byDOSAndPatientNm")
					|| requestType.equals("byDOSAndSubscriberId") || requestType.equals("byDOSAndClmNo")
					|| requestType.equals("byDOPAndClaimNo") || requestType.equals("byDOSAndPtntNm")) {
				if (payments[i].getClaimDate() != null)
					innerMap.put("Claim Date", Helper.changeDateFormatSeperator(
							Helper.changeDateFormat(payments[i].getClaimDate(), "yyyy-MM-dd", "MM-dd-yyyy")));
				else
					innerMap.put("Claim Date", "");
			} else
				// if(payments[i].getClaimDate()!=null)
				// innerMap.put("Claim
				// Date",Helper.changeDateFormatSeperator(Helper.changeDateFormat(payments[i].getClaimDate(),"yyyy-MM-dd",
				// "MM-dd-yyyy")));
				// else
				innerMap.put("Payment Date", Helper.changeDateFormatSeperator(
						Helper.changeDateFormat(payments[i].getPaymentMadeOn(), "yyyy-MM-dd", "MM-dd-yyyy")));
			...
			if (!(requestType.equals("byElectronicPaymentNo") || requestType.equals("byCheckNo")
					|| requestType.equals("byDOPAndNpi") || requestType.equals("byDOSAndNpi"))) {
				if (patientName != "")
			...
			if (requestType.equals("byElectronicPaymentNo") || requestType.equals("byDOPAndNpi")
					|| requestType.equals("byCheckNo") || requestType.equals("byDOSAndNpi")) {
			...
			if (!(requestType.equals("byElectronicPaymentNo") || requestType.equals("byDOPAndNpi")
				...
				if (payments[i].getClaimIdentifier() != null) {
					...
					if (payments[i].getClaimAmount().getAmmount() == 0.0
							|| payments[i].getClaimAmount().getAmmount() == 0.00)
			...
			// innerMap.put("Payment
			// Status".trim(),payments[i].getPaymentStatusCode().getDescription());
			// else
	...
	public LinkedHashMap<String, String> getPaymentDetailsFromFISLForCSR(Object FISLResponse)
			throws JAXBException, IOException, SAXException, ParserConfigurationException, ParseException {
		int totalPayments;
		LinkedHashMap<String, String> innerMap = null;
		Map<String, LinkedHashMap<String, String>> outerMap = new LinkedHashMap<String, LinkedHashMap<String, String>>();
		...
		// if((((EpsPaymentsSummarySearchResponse)
		// FISLResponse).getData().getTotalCount())>30)
		// totalPayments=30;
		// else
		totalPayments = payments.length;
		...
		for (int i = 0; i < totalPayments; i++) {
			...
			if (payments[i].getClaimIdentifier() != null) {
				...
				if (payments[i].getClaimAmount().getAmmount() == 0.0
						|| payments[i].getClaimAmount().getAmmount() == 0.0)
					innerMap.put("Claim Amount", "$" + "0.00");
				else {
					DecimalFormat decimalFormat = new DecimalFormat("0.00");
					String amount = decimalFormat.format((payments[i].getClaimAmount().getAmmount()));
			...
			innerMap.put("Type", payments[i].getPayeePaymentMethod().getPaymentMethodCode().getCode());
			// innerMap.put("Payment Status/Trace
			// Number",payments[i].getPaymentStatusCode().getDescription());
			innerMap.put("Market Type", getDisplayMarketType(payments[i].getPaymentTypeIndicator()));
	...
	public List<String> getDetailsFromFISL(EpsPaymentsSummarySearchResponse searchResponse, String colName)
			throws ParseException, JAXBException, IOException, SAXException, ParserConfigurationException {

		List<String> list = new ArrayList<String>();
		int totalPayments;

		EpsConsolidatedClaimPaymentSummaries[] payments = ((EpsPaymentsSummarySearchResponse) searchResponse).getData()
				.getEpsConsolidatedClaimPaymentSummaries();
		totalPayments = payments.length;
		...
		if (totalPayments > 0) {
			...
			case "Payment Date":
				for (int i = 0; i < totalPayments; i++)
					if (payments[i].getClaimDate() != null)
						list.add(Helper.changeDateFormatSeperator(
								Helper.changeDateFormat(payments[i].getClaimDate(), "yyyy-MM-dd", "MM-dd-yyyy")));
				break;
			case "NPI":
				for (int i = 0; i < totalPayments; i++)
			...
			case "Amount":
				for (int i = 0; i < totalPayments; i++)
					if (payments[i].getClaimIdentifier() != null && payments[i].getClaimAmount().getAmmount() == 0.0
							|| payments[i].getClaimAmount().getAmmount() == 0.0)
						list.add("$" + "0.00");
					else {
						DecimalFormat decimalFormat = new DecimalFormat("0.00");
	...
	/**
	 * Author: Mohammad Khalid This method clicks on the EPRA PDF link to download
	 * the file
	 **/
	public RemittanceDetail clickEPRAPDFlink_Enabled_SR(String srchType) {
		...
		for (int pageNum = 1; pageNum <= totalNoOfPages; pageNum++) {
			if (pageNum % 10 == 1) {
				flag=find_And_Click_835PDF_Icon_Enabled_SR();
				if(flag==true)
					break;
			} else if (pageNum % 10 == 0) {
				String xpathNextPage = "(//a[@class='pageNo' and contains(text(),'" + (pageNum) + "')])[1]";
				Element.click(testConfig, TestBase.driver.findElement(By.xpath(xpathNextPage)),
						"The Page Number clicked is: " + (pageNum), 5);
				flag=find_And_Click_835PDF_Icon_Enabled_SR();
				if(flag==true)
					break;
	...
	public SearchRemittance clickEpraPDFlink(String srchType) {
		...
		if (srchType.equals("byDOPAndNpi") || srchType.equals("EPRADOPAndNpi")) {
			...
			System.setProperty("paymentNum", expectedPaymntNo);
			// String paymentNum =
			// System.getProperty("ELECTRONIC_PAYMENT_NUMBER");
			searchResultRows = Element.findElements(testConfig, "xpath",
		...
		for (int pageNo = 1; pageNo <= totalNoOfPages; pageNo++) {
					...
					// if(actualPaymntNo.equals(expectedPaymntNo))
					{
						...
						if (srchType.equals("byDOPAndNpi") || srchType.equals("EPRADOPAndNpi"))
							lnkEpraPdf = Element.findElement(testConfig, "xpath",
									"//*[@id='searchRemittanceResultsForm']/table/tbody/tr[8]/td/table/tbody/tr/td/table/tbody/tr["
											+ (i + 1) + "]/td[4]/../td[8]/table/tbody/tr/td[3]/span[3]//img");
						else if (srchType.equals("byElectronicPaymentNo") || srchType.equals("EPRAElectronicPaymentNo"))
							lnkEpraPdf = Element.findElement(testConfig, "xpath",
									"//form[@id='searchRemittanceResultsForm']/table//tr[7]/td/table/tbody/tr/td/table/tbody/tr["
											+ (i + 1) + "]/td[4]/../td[8]/table/tbody/tr/td[3]/span[3]//img");
							...
							lnkEpraPdf = Element.findElement(testConfig, "xpath",
									"//form[@id='paymentsummaryform']/table[1]/tbody/tr[5]/td/table/tbody/tr[2]/td/table/tbody/tr/td/table/tbody/tr["
											+ (i + 1) + "]/td[11]/table/tbody/tr/td[3]/span/span/a/img");

						Browser.wait(testConfig, 5);
						// Browser.browserRefresh(testConfig);
						// Browser.wait(testConfig, 3);
						if (srchType.equals("viewPayments"))
							Element.waitForPresenceOfElementLocated(testConfig, By.xpath(
									"//form[@id='paymentsummaryform']/table[1]/tbody/tr[5]/td/table/tbody/tr[2]/td/table/tbody/tr/td/table/tbody/tr["
											+ (i + 1) + "]/td[11]/table/tbody/tr/td[3]/span/span/a/img"),
									60);
			...
			if (found == true)
				...
				Element.findElement(testConfig, "xpath",
						"//body/div/table/tbody/tr/td/table/tbody/tr/td/form/table/tbody/tr[7]/td[1]/span[1]/a["
								+ pageToBeClicked + "]")
						.click();
				// Element.findElement(testConfig,"xpath",".//*[@id='paymentsummaryform']/table[1]/tbody/tr[4]/td/span//a[contains(text(),"
				...
				Log.Warning("Could not find nonEpra payment on any of the pages, please execute test case manually",
						testConfig);
		}
	...
	public SearchRemittance verifyUsrEvntLog() {
		...
		// is:"+testConfig.getRunTimeProperty("id"));
		Map data = DataBase.executeSelectQuery(testConfig, sqlRowNo, 1);

		Log.Comment("The Data from User Event Log Table is:" + data);
		Helper.compareContains(testConfig, "EPRA is downloaded", testConfig.getRunTimeProperty("id"),
				data.get("EVNT_MSG").toString());
		// Helper.compareContains(testConfig, "EPRA is
		// downloaded",testConfig.getRunTimeProperty("ELECTRONIC_PAYMENT_NUMBER"),
		// data.get("EVNT_MSG").toString());
		// Helper.compareContains(testConfig, "EPRA is downloaded",
		// "DataAccess:SUCCESS:Protected Data Access: :Payment
		// Summary(PrintEPRA):P:", data.get("EVNT_MSG").toString());
		Browser.wait(testConfig, 3);
		if (testConfig.getRunTimeProperty("TobeDeleted") != null
				&& testConfig.getRunTimeProperty("TobeDeleted").equals("PROV"))
			DataBase.executeDeleteQuery(testConfig, 25);
		if (testConfig.getRunTimeProperty("TobeDeleted") != null
				&& testConfig.getRunTimeProperty("TobeDeleted").equals("BS"))
	...
	public SearchRemittance verifyUsrEvntLogForSrchRemit(String userType) {
		...
		// testConfig.getRunTimeProperty("id"),
		// data.get("EVNT_MSG").toString());
		Helper.compareContains(testConfig, "EPRA is downloaded", System.getProperty("id"),
				data.get("EVNT_MSG").toString());
		Browser.wait(testConfig, 3);
		if (testConfig.getRunTimeProperty("TobeDeleted") != null
				&& testConfig.getRunTimeProperty("TobeDeleted").equals("PROV"))
	...
	public SearchRemittance verifyUsrEvntLogRemitDetailCSR(String userType) {
		...
		Helper.compareContains(testConfig, "EPRA is downloaded", testConfig.getRunTimeProperty("id"),
				data.get("EVNT_MSG").toString());
		Helper.compareContains(testConfig, "Contains TIN", testConfig.getRunTimeProperty("tin"), data.get("EVNT_MSG").toString());
	...
	public SearchRemittance verifyfileinNASDrive() throws SftpException, JSchException {
		...
		Log.Comment("/home/ppsstg/stg/erapdf/pdf/" + System.getProperty("tin").substring(0, 2) + "/"
				+ System.getProperty("tin") + "/", System.getProperty("PDFFileName"));
		Log.Comment("/home/ppsstg/stg/erapdf/pdf/" + System.getProperty("tin").substring(0, 2) + "/"
				+ System.getProperty("tin") + "/" + System.getProperty("PDFFileName"));

		System.out.println((obj_FTPaccess.IsFileAvailable("/home/ppsstg/stg/erapdf/pdf/"
				+ System.getProperty("tin").substring(0, 2) + "/" + System.getProperty("tin") + "/",
				System.getProperty("PDFFileName"))));

		if (obj_FTPaccess.IsFileAvailable("/home/ppsstg/stg/erapdf/pdf/" + System.getProperty("tin").substring(0, 2)
				+ "/" + System.getProperty("tin") + "/" + "", System.getProperty("PDFFileName"))) {
			System.out.print("*(((------File Available*");
			System.out.print("*(((------File PDFFileName*" + System.getProperty("PDFFileName") + "---");
			obj_FTPaccess.DeleteFile("/home/ppsstg/stg/erapdf/pdf/" + System.getProperty("tin").substring(0, 2) + "/"
					+ System.getProperty("tin") + "/", System.getProperty("PDFFileName"));
	...
	public ViewPayments verifyppraStatus(String expectedStatus) {
		...
		Helper.compareEquals(testConfig,
				"Status in Epra status for payment number : " + ppraStatusTbl.get("CONSL_PAY_NBR"), expectedStatus,
				ppraStatusTbl.get("REQ_STS").toString());
		return this;
	...
	public SearchRemittance validateHyfen() {
		if (Element.findElement(testConfig, "xpath", "//*[@id='SearchCriteria']/table/tbody/tr/td").getText()
				.contains("-"))
	...
	public ViewPayments verifyPrintSearchRemitPage(String requestType)
		...
		EpsPaymentsSummarySearchResponse searchResponse = (EpsPaymentsSummarySearchResponse) getFISLResponse(
				requestType);
		Helper.compareMaps(testConfig, "Print Search Remit Details Comparison",
				getSRDetailsFromFISL(requestType, searchResponse), getSRDetailsFromUI(requestType));
	...
	/**
	 * Author: Mohammad Khalid This method finds on the Payment Number on the Search
	 * Remittance page
	 **/

	public boolean find_And_Click_PaymentNumber_SR() {
		...
		return flag_ClickedPaymentNumber;
	}

	/**
	 * Author: Mohammad Khalid Method to find on PDF Link on SR page
	 */

	public boolean find_And_Click_835PDF_Link_SR() {
		String expectedPaymntNo = null;
		boolean flag_Clicked_835Link=false;
		List<WebElement> ls_SinglePage = Element.findElements(testConfig, "xpath",
				"//tr[@class='search-remittance__table_header']/../tr");
		expectedPaymntNo = System.getProperty("ELECTRONIC_PAYMENT_NUMBER");
		for (int i = 1; i < ls_SinglePage.size(); i++) {
			String xpath_PaymentNum = "//tr[@class='search-remittance__table_header']/../tr[" + (i + 1) + "]/td[4]/a";
			String xpath_835PDF = "//tr[@class='search-remittance__table_header']/../tr[" + (i + 1)
					+ "]/td[8]/table/tbody/tr/td[3]/span[1]/a";
			String xpath_835PDF_AfterClick = "//tr[@class='search-remittance__table_header']/../tr[" + (i + 1)
					+ "]/td[8]/table/tbody/tr/td[3]/span[2]";
			...
			if (payNumber_SR.getText().trim().equalsIgnoreCase(expectedPaymntNo)) {
				Element.clickByJS(testConfig, f_835_PDF_SR,
						"835 PDF link on SR page for Payment Number: " + payNumber_SR.getText().trim());

				String oldWindow = Browser.switchToNewWindow(testConfig, "EPRADisplayWindow");
				Browser.switchToParentWindow(testConfig, oldWindow);

				Helper.compareEquals(testConfig, "ePRA PDF link Hover Message after the click",
						"ePRA in process, please wait for completion",
						f_835_PDF_SR_AfterClick.getAttribute("title").trim());
				flag_Clicked_835Link=true;
				break;
			}
		}
		return flag_Clicked_835Link;
	}

	/**
	 * Author: Mohammad Khalid Method to find on PDF icon enabled on SR page
	 */

	public boolean find_And_Click_835PDF_Icon_Enabled_SR() {
		String expectedPaymntNo = null;
		boolean flag_Clicked_835PDF_Icon=false;
		List<WebElement> ls_SinglePage = Element.findElements(testConfig, "xpath",
				"//tr[@class='search-remittance__table_header']/../tr");
		expectedPaymntNo = System.getProperty("ELECTRONIC_PAYMENT_NUMBER");
		for (int i = 1; i < ls_SinglePage.size(); i++) {
			String xpath_PaymentNum = "//tr[@class='search-remittance__table_header']/../tr[" + (i + 1) + "]/td[4]/a";
			String xpath_835PDF = "//tr[@class='search-remittance__table_header']/../tr[" + (i + 1)
					+ "]/td[8]/table/tbody/tr/td[3]/span[3]/a";
			WebElement payNumber_SR = Element.findElement(testConfig, "xpath", xpath_PaymentNum);
			WebElement f_835_PDF_SR = Element.findElement(testConfig, "xpath", xpath_835PDF);
			if (payNumber_SR.getText().trim().equalsIgnoreCase(expectedPaymntNo)) {
				wait.until(ExpectedConditions.elementToBeClickable(f_835_PDF_SR));
				Element.clickByJS(testConfig, f_835_PDF_SR,
						"835 PDF Icon on SR page for Payment Number: " + payNumber_SR.getText().trim());
				flag_Clicked_835PDF_Icon = true;
				break;
			}
		}
		
		return flag_Clicked_835PDF_Icon;
	}
	...
	public RemittanceDetail clickPaymentNumber_SR_Page(String srchType) {
		...
		for (int pageNum = 1; pageNum <= totalNoOfPages; pageNum++) {
			if (pageNum % 10 == 1) {
				flag = find_And_Click_PaymentNumber_SR();
				if(flag==true)
					break;
			} else if (pageNum % 10 == 0) {
				String xpathNextPage = "(//a[@class='pageNo' and contains(text(),'" + (pageNum) + "')])[1]";
	...
	/**
	 * Author: Mohammad Khalid Method to click on PDF Link on SR page
	 */
	...
	public RemittanceDetail click_835PDF_EPRA_SR(String srchType) {
		...
		for (int pageNum = 1; pageNum <= totalNoOfPages; pageNum++) {
			if (pageNum % 10 == 1) {
				flag=find_And_Click_835PDF_Link_SR();
				if(flag==true)
					break;
			} else if (pageNum % 10 == 0) {
				String xpathNextPage = "(//a[@class='pageNo' and contains(text(),'" + (pageNum) + "')])[1]";
	...
	public int getNumberOfPages() {
		int noOfPages = 0;
		if (getRecordCountFromUI_EPRA_SR() != null) {
			noOfPages = Integer.parseInt(getRecordCountFromUI_EPRA_SR());
		} else
			noOfPages = 1;
